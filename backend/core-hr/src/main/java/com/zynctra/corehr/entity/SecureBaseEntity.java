...
/**
 * Secure base entity for core-hr domain.
 * 
 * SECURITY INVARIANTS:
 * - tenantId IMMUTABLE (set once, never changed)
 * - Soft delete only (deleted flag + retention date)
 * - Automatic tenant scoping via Hibernate @Filter
...
public abstract class SecureBaseEntity {
    ...
    /**
     * GDPR retention date. After this date, data is physically purged.
     * Set on termination/soft-delete.
     */
    ...
    protected void onUpdate() {
        this.updatedAt = Instant.now();
        // tenantId CANNOT change — enforced by updatable=false
    }
