// cat > /mnt/user-data/outputs/PayrollCalculator.tsx << 'EOF'
/**
...
export const PayrollCalculator: React.FC<PayrollCalculatorProps> = ({ onCalculate }) => {
  ...
  const calculatePayroll = (): PayrollOutput => {
    ...
    const deductionsTotal = Object.values(input.deductions || {}).reduce((a, b) => a + b, 0);
        ...
        <div className="bg-gradient-to-br from-cyan-50 to-blue-50 dark:from-cyan-900/20 dark:to-blue-900/20 rounded-lg border border-cyan-200 dark:border-cyan-800 p-6 space-y-4">
          ...
          <div className="space-y-3">
            <div className="flex justify-between text-sm">
              <span>Gross Pay:</span>
              <span className="font-semibold">${output.grossPay.toLocaleString()}</span>
            </div>
            <div className="flex justify-between text-sm border-t border-cyan-200 dark:border-cyan-800 pt-3">
              <span>Income Tax:</span>
              <span className="font-semibold text-red-600">${output.taxAmount.toLocaleString()}</span>
            </div>
            <div className="flex justify-between text-sm">
